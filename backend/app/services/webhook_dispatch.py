...
def dispatch_event(db: Session, *, agency_id: int, event_type: str, payload: dict) -> list[dict]:
    ...
    for wh in webhooks:
        if event_type in (wh.events or []):
            result = _deliver(wh, event_type, payload, db)
    ...
    try:
        ...
        if resp.status_code >= 400:
            ...
            if retries < MAX_RETRIES:
                ...
                time.sleep(RETRY_DELAY_SECONDS[min(retries, len(RETRY_DELAY_SECONDS) - 1)])
                return _deliver(wh, event_type, payload, db, retries=retries + 1)
